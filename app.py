...
with tab_predict:
    ...
    # Optional ML deps (guarded import). Falls back to EB if sklearn missing.
    _SK_OK = True
    try:
    ...
    if MISSING:
        st.error(f"Predictivity tab needs these columns present: {MISSING}")
        st.stop()
    ...
    def _build_features(raw: pd.DataFrame, as_of: date) -> pd.DataFrame:
        ...
        # Targets (based on Payment Received Date)
    ...
    def _predict_ML(X: pd.DataFrame, as_of: date):
        ...
        def _pipe():
            ...
            pre = ColumnTransformer(transf) if transf else "passthrough"
            return Pipeline([("pre", pre), ("clf", GradientBoostingClassifier(random_state=42))])

        models={}
        for key, target_col in {"thismonth":"y_thismonth", "nextmonth":"y_nextmonth"}.items():
            y = X.loc[train_mask, target_col]
        ...
        if len(idx) > 0:
            ...
            for k, m in models.items():
                ...
                totals[k] = float(np.sum(probs))
                data[f"p_{k}"] = probs
            # Optional columns for readability
            if "Country" in X.columns: data["Country"] = X.loc[idx,"Country"].astype(str).values
            if "JetLearn Deal Source" in X.columns: data["JetLearn Deal Source"] = X.loc[idx,"JetLearn Deal Source"].astype(str).values
            per_deal = pd.DataFrame(data)
        return totals, per_deal

    def _predict_EB(X: pd.DataFrame, as_of: date):
        """Cohort-based fallback: estimate r0 (M0) & r1 (M1) by (Source, Country)."""
        this_m0 = _mstart(as_of)
    ...
    if run:
        ...
        k1, k2 = st.columns(2)
        k1.metric(f"Predicted This Month ({this_m0:%b %Y})", f"{int(round(totals['thismonth'])):,}")
        k2.metric(f"Predicted Next Month ({next_m0:%b %Y})", f"{int(round(totals['nextmonth'])):,}")
